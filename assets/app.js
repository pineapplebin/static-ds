...
// It may cause some legal issues if we allow exporting the save data in plain text.
...
async function uiSaveRestore() {
  ...
  if (file.name.endsWith(".sav")) {
    ...
    if (origSave.length <= 0) {
      alert(
        "You have to save in the game at least once, before importing .sav file."
      );
      return;
    }
  ...
  localforage.setItem("sav-" + gameID, u8Arr).then(() => {
    ...
    setTimeout(() => {
      location.reload();
      // document.write("Please close and reopen this app.");
    }, 500);
  });
}
...
function uiAdjustVKLayout() {
  ...
  var offTop = Math.min(
    fbSize[0][1] + fbSize[1][1],
    window.innerHeight - Math.ceil(baseSize * 3.62)
  );
  var offLeft = 0;
  ...
  vkh = baseSize * 1.0;
  offLeft = 0;
  $id("vk-stick").style = config.useDPad
    ? "display:none;"
    : makeVKStyle(
        offTop + abxyHeight / 2 - vkh / 2,
  ...
  $id("vk-dpad-1").style = config.useDPad
    ? makeVKStyle(dpadY + dpadH / 3, dpadX, dpadW, dpadH / 3, fontSize)
    : "display:none;";
  $id("vk-dpad-2").style = config.useDPad
    ? makeVKStyle(dpadY, dpadX + dpadW / 3, dpadW / 3, dpadH, fontSize)
    : "display:none;";
  vkw = baseSize * 0.4;
  vkh = baseSize * 0.4;
  fontSize = baseSize * 0.2;
  vkMap["select"].style = makeVKStyle(
    offTop + abxyHeight - vkh,
    window.innerWidth / 2 - vkw * 1.5,
    vkw,
    vkh,
    fontSize
  );
  vkMap["start"].style = makeVKStyle(
    offTop + abxyHeight - vkh,
    window.innerWidth / 2 + vkw * 0.5,
    vkw,
    vkh,
    fontSize
  );
  ...
  for (var i = 0; i < event.touches.length; i++) {
    ...
    if (config.useDPad) {
      if (isPointInRect(t.clientX, t.clientY, vkDPadRect)) {
        var xgrid = Math.floor(
          ((t.clientX - vkDPadRect.x) / vkDPadRect.width) * 3
        );
        var ygrid = Math.floor(
          ((t.clientY - vkDPadRect.y) / vkDPadRect.height) * 3
        );
      ...
      if (
        tid === stickTouchID ||
        (dom == vkMap["stick"] && tid != tpadTouchID)
      ) {
        stickPressed = true;
    ...
    if (
      tid === tpadTouchID ||
      (isPointInRect(t.clientX, t.clientY, tsRect) && !k)
    ) {
      isDown = true;
...
function gpuInitWithCanvas(canvas) {
  ...
  // Set uniform.
  canvas.outResolutionUniformLocation = gl.getUniformLocation(
    program,
    "u_outResolution"
  );
  var inResolutionUniformLocation = gl.getUniformLocation(
    program,
    "u_inResolution"
  );
...
function gpuDraw(canvas, idata) {
  ...
  gl.viewport(0, 0, canvas.width, canvas.height);
  gl.uniform2f(
    canvas.outResolutionUniformLocation,
    canvas.width,
    canvas.height
  );
